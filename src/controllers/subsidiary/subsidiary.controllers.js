const Subsidiary = require('../models/subsidiary'); // Importa el modelo Sucursales

const getAllSubsidiary = async (req, res) => { // Función para obtener todas las sucursales
  try {
...
const getSubsidiaryById = async (req, res) => { // Función para obtener una sucursal por ID
  try {
...
const createSubsidiary = async (req, res) => { // Función para crear una nueva sucursal
  try {
...
const updateSubsidiary = async (req, res) => { // Función para actualizar una sucursal por ID
  try {
    const subsidiary = await Subsidiary.findByPk(req.params.id); // Busca la sucursal por ID
    if (!subsidiary) return res.status(404).send("Subsidiary not found"); // Si no se encuentra, envía un error 404
    const { adress, position } = req.body; // Obtiene los datos del cuerpo de la petición
...
const deleteSubsidiary = async (req, res) => { // Función para eliminar una sucursal por ID
  try {
    const subsidiary = await Subsidiary.findByPk(req.params.id); // Busca la sucursal por ID
